...
`realesr-general-x4v3` model (fast, BSD-3-Clause) through
...
- 2× / 3× / 4× upscaling with Real-ESRGAN — new detail is reconstructed,
  not just stretched (2× is the default)
- **AI strength** slider (0–100%, default 50%): blends the AI result with
  a plain high-quality upscale, so output can stay close to the original —
...
- **First upscale is slow to start**: the ONNX runtime + model
  (x4-v3 ~5 MB) download and initialize the first time.
...
- **The AI model is photo-tuned**: on screenshots, UI text and sharp
